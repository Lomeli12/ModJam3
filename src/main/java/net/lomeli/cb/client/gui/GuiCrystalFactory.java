...
public class GuiCrystalFactory extends GuiContainer {
    ...
    public GuiCrystalFactory(EntityPlayer player, TileCrystalFactory tileEntity) {
        super(new ContainerCrystalFactory(player.inventory, tileEntity));
        tile = tileEntity;
        this.xSize += 18;
        this.ySize += 55;
    }
    ...
    public void initGui() {
        ...
        this.buttonList.add(smelt1 = new GuiButtonCrystal(0, guiLeft + 31, guiTop + 107,
                fontRendererObj.getStringWidth(smelt) + 10, 20, smelt));
        this.buttonList.add(smelt2 = new GuiButtonCrystal(1, guiLeft + 73, guiTop + 107,
                fontRendererObj.getStringWidth(smelt) + 10, 20, smelt));
        this.buttonList.add(smelt3 = new GuiButtonCrystal(2, guiLeft + 114, guiTop + 107,
                fontRendererObj.getStringWidth(smelt) + 10, 20, smelt));
        this.buttonList.add(crystalize = new GuiButtonCrystal(3, guiLeft + 135, guiTop + 70,
                fontRendererObj.getStringWidth(form) + 4, 20, form));
    }
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        String s = StatCollector.translateToLocal(tile.getInventoryName());
        this.fontRendererObj.drawString(s, (this.xSize / 2 - this.fontRendererObj.getStringWidth(s) / 2) - 50, 6, 4210752);
        this.fontRendererObj.drawString(I18n.format("container.inventory", new Object[0]), 5, this.ySize - 95 + 2, 4210752);
        ...
        if(tile.getCurrentCharge() > 0) {
            ...
            drawToolTipOverArea(mouseX, mouseY, 7, 26, 22, 102, text, fontRendererObj);
        }

        if(tile.tanks[0].getFluidAmount() > 0
                || (tile.tanks[0].getFluid() != null && tile.tanks[0].getFluid().getFluid() != null)) {
            text.clear();
        ...
        if(tile.tanks[1].getFluidAmount() > 0
                || (tile.tanks[1].getFluid() != null && tile.tanks[1].getFluid().getFluid() != null)) {
            text.clear();
            ...
            drawToolTipOverArea(mouseX, mouseY, 82, 39, 97, 82, text, fontRendererObj);
        }else {
            text.clear();
            text.add("Empty");
            drawToolTipOverArea(mouseX, mouseY, 82, 39, 97, 82, text, fontRendererObj);
        ...
        if(tile.tanks[2].getFluidAmount() > 0
                || (tile.tanks[2].getFluid() != null && tile.tanks[2].getFluid().getFluid() != null)) {
            text.clear();
            ...
            drawToolTipOverArea(mouseX, mouseY, 116, 39, 131, 82, text, fontRendererObj);
        }else {
            text.clear();
            text.add("Empty");
            drawToolTipOverArea(mouseX, mouseY, 116, 39, 131, 82, text, fontRendererObj);
    ...
    public void drawToolTipOverArea(int mouseX, int mouseY, int minX, int minY, int maxX, int maxY, List<String> list,
            FontRenderer font) {
