...
public class AbilityAnimalLove extends CrystalAbility {
    ...
    public void enviromentalEffect(World worldObj, int x, int y, int z, Random rand) {
        if(rand.nextInt(10000) < 50) {
            ...
            if(list.size() > 0) {
                try {
                    EntityLivingBase entity = (EntityLivingBase) worldObj
                            .getChunkProvider()
                            .getPossibleCreatures(EnumCreatureType.creature, x, y, z)
                            .get(rand.nextInt(worldObj.getChunkProvider()
                                    .getPossibleCreatures(EnumCreatureType.creature, x, y, z).size()));
                    if(entity != null) {
